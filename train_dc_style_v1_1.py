...
class Generator(nn.Module):
    def __init__(self,
        ...
        self.linear = Projection(nz, nz, 4*4*nfeats*16, [nfeats*16, 4, 4], bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        #(nfeats*16) x 4 x 4

        self.conv1 = UpConvBlock(nz, nfeats*16, nfeats*16, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        #(nfeats*16) x 8 x 8

        self.conv2 = UpConvBlock(nz, nfeats*16, nfeats*8, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        #(nfeats*8) x 16 x 16

        self.conv3 = UpConvBlock(nz, nfeats*8, nfeats*4, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        #(nfeats*4) x 32 x 32

        self.conv4 = UpConvBlock(nz, nfeats*4, nfeats*2, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        #(nfeats*2) x 64 x 64

        self.conv5 = UpConvBlock(nz, nfeats*2, nfeats*1, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        #(nfeats*1) x 128 x 128

        self.conv6 = nn.Conv2d(nfeats*1, nchannels, 1, 1, 0, bias=bias)
        if spectral_norm:
            self.conv6 = SN(self.conv6)
        #(nchannels) x 128 x 128
    ...
    def forward(self, x):
        ...
        '''
        G mapping torch.Size([128, 128])
        G linear torch.Size([128, 512, 4, 4])
        G conv1 torch.Size([128, 512, 8, 8])
        G conv2 torch.Size([128, 256, 16, 16])
        G conv3 torch.Size([128, 128, 32, 32])
        G conv4 torch.Size([128, 64, 64, 64])
        G conv5 torch.Size([128, 32, 128, 128])
        G tanh torch.Size([128, 3, 128, 128])
        '''
...
class Discriminator(nn.Module):
    def __init__(self, nchannels, nfeats):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.BatchNorm2d(d),
            nn.LeakyReLU(0.2), # (nfeats) x 128 x 128

            SN(nn.Conv2d(d, d, 3, 2, 1, bias=False)),
            nn.BatchNorm2d(d),
            nn.LeakyReLU(0.2), # (nfeats) x 64 x 64
        )
        # (nfeats) x 64 x 64

        self.conv2 = nn.Sequential(
            SN(nn.Conv2d(d, d*2, 3, 1, 1, bias=False)),
            nn.BatchNorm2d(d*2),
            nn.LeakyReLU(0.2), # (nfeats*2) x 64 x 64

            SN(nn.Conv2d(d*2, d*2, 3, 2, 1, bias=False)),
            nn.BatchNorm2d(d*2),
            nn.LeakyReLU(0.2), # (nfeats*2) x 32 x 32
        )
        # (nfeats*2) x 32 x 32

        self.conv3 = nn.Sequential(
            SN(nn.Conv2d(d*2, d*4, 3, 1, 1, bias=False)),
            nn.BatchNorm2d(d*4),
            nn.LeakyReLU(0.2), # (nfeats*4) x 32 x 32

            SN(nn.Conv2d(d*4, d*4, 3, 2, 1, bias=False)),
            nn.BatchNorm2d(d*4),
            nn.LeakyReLU(0.2), # (nfeats*4) x 16 x 16
        )
        # (nfeats*4) x 16 x 16

        self.conv4 = nn.Sequential(
            SN(nn.Conv2d(d*4, d*8, 3, 1, 1, bias=False)),
            nn.BatchNorm2d(d*8),
            nn.LeakyReLU(0.2), # (nfeats*8) x 16 x 16

            SN(nn.Conv2d(d*8, d*8, 3, 2, 1, bias=False)),
            nn.BatchNorm2d(d*8),
            nn.LeakyReLU(0.2), # (nfeats*8) x 8 x 8
        )
        # (nfeats*8) x 8 x 8

        self.conv5 = nn.Sequential(
            SN(nn.Conv2d(d*8, d*16, 3, 1, 1, bias=False)),
            nn.BatchNorm2d(d*16),
            nn.LeakyReLU(0.2), # (nfeats*16) x 8 x 8

            SN(nn.Conv2d(d*16, d*16, 3, 2, 1, bias=False)),
            nn.BatchNorm2d(d*16),
            nn.LeakyReLU(0.2), # (nfeats*16) x 4 x 4
        )
        # (nfeats*16) x 4 x 4

        self.conv6 = nn.Sequential(
            SN(nn.Conv2d(d*16, d*16, 3, 1, 1, bias=False)),
            nn.BatchNorm2d(d*16),
            nn.LeakyReLU(0.2), # (nfeats*16) x 4 x 4
        ...
        self.linear = SN(nn.Linear(d*16, 1, bias=False))
    ...
    def forward(self, x):
        ...
        x = torch.sum((F.leaky_relu(x,0.2)).view(x.shape[0],-1,4*4), dim=2) #GlobalSumPool ->(*,16ch)
        x = self.linear(x)

        '''
        D conv1 torch.Size([32, 32, 64, 64])
        D conv2 torch.Size([32, 64, 32, 32])
        D conv3 torch.Size([32, 128, 16, 16])
        D conv4 torch.Size([32, 256, 8, 8])
        D conv5 torch.Size([32, 512, 4, 4])
        D conv6 torch.Size([32, 512, 4, 4])
        D linear torch.Size([32, 1])
        '''
...
def print_params():
    ...
    printBoth(LOG, 'BIAS = {}'.format(BIAS))
...
def generate_images(model_path, dir_images_output, num_images=10000, batch_size=1000, truncated=None, device='cuda'):
    # load model
    netG = Generator(NZ, NGF, 3, BIAS, SPECTRAL_NORM, NORMALIZATION, RANDOM_NOISE, USE_STYLE, PIXEL_NORM).to(device)
