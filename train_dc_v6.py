...
def generate_images(model_path, dir_images_output, num_images=10000, batch_size=1000, truncated=None):
    ...
    for batch in range(int(num_images/batch_size)):
        ...
        if truncated is not None:
            cont = True
            while cont:
                z = np.random.randn(100*batch_size*Config.NZ)
                z = z[np.where(abs(z)<truncated)]
                if len(z)>=batch_size*Config.NZ:
                    cont = False

            z = torch.from_numpy(z[:batch_size*Config.NZ]).view(batch_size,Config.NZ,1,1)
            z = z.float().to(device)
        else:
            z = torch.randn(batch_size, Config.NZ, 1, 1, device=device)
...
def generate_images_truncated(model_path, dir_images_output, num_images=10000, batch_size=100, truncated=1.0):
    #device = 'cpu'
    device = 'cuda'
