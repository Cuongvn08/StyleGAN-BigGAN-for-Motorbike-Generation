...
MANUAL_SEED = 6718
PATH_MODEL_G = '/data/cuong/result/motobike/dc_style_v1/56_G.pth'
PATH_MODEL_D = '/data/cuong/result/motobike/dc_style_v1/56_D.pth'
...
class Generator(nn.Module):
    def __init__(self,
        ...
        self.linear = Projection(nz, nz, 8*8*nfeats, [nfeats, 8, 8], bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm) #(nfeats*16) x 4 x 4

        self.conv2 = UpConvBlock(nz, nfeats, nfeats//2, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm) #(nfeats*8) x 8 x 8

        self.conv3 = UpConvBlock(nz, nfeats//2, nfeats//4, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm) #(nfeats*4) x 16 x 16

        self.conv4 = UpConvBlock(nz, nfeats//4, nfeats//8, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm) #(nfeats*2) x 32 x 32

        self.conv5 = UpConvBlock(nz, nfeats//8, nfeats//16, 4, 2, 1, bias,
            spectral_norm, normalization, random_noise, use_style, use_pixel_norm) #(nfeats*1) x 64 x 64

        self.conv6 = nn.Conv2d(nfeats//16, nchannels, 1, 1, 0, bias=bias) #(nchannels) x 128 x 128
        if spectral_norm:
...
class Discriminator(nn.Module):
    def __init__(self, nchannels, nfeats):
        ...
        self.from_rgb = nn.Sequential(
            SN(nn.Conv2d(nchannels, d//16, 1, 1, 0, bias=False)),
            nn.BatchNorm2d(d//16),
            nn.LeakyReLU(0.2)
        )
