...
class Generator(nn.Module):
    def __init__(self, n_feat, codes_dim=NZ):
        super().__init__()
    ...
    def forward(self, z):
        ...
        x = self.res1(x, z)                 #->(*,16ch,8,8)
        x = self.res2(x, z)                 #->(*,8ch,16,16)
        x = self.res3(x, z)                 #->(*,4ch,32,32)
        x = self.res4(x, z)                 #->(*,2ch,64,64)
        x = self.res5(x, z)                 #->(*,1ch,128,128)
...
def run(lr_G=3e-4,lr_D=6e-4, beta1=0.0, beta2=0.999, nz=120, epochs=2,
    ...
    netG = Generator(n_feat=NGF, codes_dim=nz).to(device)
