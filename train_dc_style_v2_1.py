...
class UpConvBlock(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_pixel_norm=False):
        super().__init__()
        self.conv1 = nn.ConvTranspose2d(in_channel, out_channel, kernel, padding, stride, bias=bias)
...
#             nn.LeakyReLU(0.2),
        ...
        self.linear = Projection(nz, nz, 8*8*nfeats*16, [nfeats*16, 8, 8], bias, spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        # state size. (nfeats*16) x 8 x 8

        self.conv1 = UpConvBlock(nz, nfeats*16, nfeats*8, 4, 2, 1, bias, spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        # state size. (nfeats*8) x 16 x 16

        self.conv2 = UpConvBlock(nz, nfeats*8, nfeats*4, 4, 2, 1, bias, spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        # state size. (nfeats*4) x 32 x 32

        self.conv3 = UpConvBlock(nz, nfeats*4, nfeats*2, 4, 2, 1, bias, spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        # state size. (nfeats*2) x 64 x 64

        self.conv4 = UpConvBlock(nz, nfeats*2, nfeats*1, 4, 2, 1, bias, spectral_norm, normalization, random_noise, use_style, use_pixel_norm)
        # state size. (nfeats*1) x 128 x 128

        self.conv5 = nn.Conv2d(nfeats, nchannels, 1, 1, 0, bias=bias)
        if spectral_norm:
            self.conv5 = SN(self.conv5)
        # state size. (nchannels) x 128 x 128
    ...
    def forward(self, x):
        ...
        '''
        G mapping = torch.Size([4, 128])
        G linear = torch.Size([4, 1024, 8, 8])
        G conv1 = torch.Size([4, 512, 16, 16])
        G conv2 = torch.Size([4, 256, 32, 32])
        G conv3 = torch.Size([4, 128, 64, 64])
        G conv4 = torch.Size([4, 64, 128, 128])
        G out = torch.Size([4, 3, 128, 128])
        '''
...
class Discriminator(nn.Module):
    def __init__(self, nchannels, nfeats):
        ...
        self.conv1 = nn.Sequential(
            SN(nn.Conv2d(nchannels, d, 3, 1, 1, bias=True)),
            nn.BatchNorm2d(d),
            nn.LeakyReLU(0.2)
        )
        # (nfeats) x 128 x 128
        ...
        self.conv2 = nn.Sequential(
            SN(nn.Conv2d(d, d*2, 4, 2, 1, bias=True)),
            nn.BatchNorm2d(d*2),
            nn.LeakyReLU(0.2),
            SN(nn.Conv2d(d*2, d*2, 3, 1, 1, bias=True)),
            nn.BatchNorm2d(d*2),
            nn.LeakyReLU(0.2)
        )
        # (2*nfeats) x 64 x 64
        ...
        self.conv3 = nn.Sequential(
            SN(nn.Conv2d(d*2, d*4, 4, 2, 1, bias=True)),
            nn.BatchNorm2d(d*4),
            nn.LeakyReLU(0.2),
            SN(nn.Conv2d(d*4, d*4, 3, 1, 1, bias=True)),
            nn.BatchNorm2d(d*4),
            nn.LeakyReLU(0.2)
        )
        # (4*nfeats) x 32 x 32
        ...
        self.conv4 = nn.Sequential(
            SN(nn.Conv2d(d*4, d*8, 4, 2, 1, bias=True)),
            nn.LeakyReLU(0.2),
            nn.BatchNorm2d(d*8),
            SN(nn.Conv2d(d*8, d*8, 3, 1, 1, bias=True)),
            nn.BatchNorm2d(d*8),
            nn.LeakyReLU(0.2)
        )
        # (8*nfeats) x 16 x 16
        ...
        self.conv5 = nn.Sequential(
            SN(nn.Conv2d(d*8, d*16, 4, 2, 1, bias=True)),
            nn.LeakyReLU(0.2),
            nn.BatchNorm2d(d*16),
        ...
        self.linear = SN(nn.Linear(8*8*d*16, 1, bias=True))
    ...
    def forward(self, x):
        ...
        '''
        D conv1 = torch.Size([32, 64, 128, 128])
        D conv2 = torch.Size([32, 128, 64, 64])
        D conv3 = torch.Size([32, 256, 32, 32])
        D conv4 = torch.Size([32, 512, 16, 16])
        D conv5 = torch.Size([32, 1024, 8, 8])
...
class Trainer:
    def __init__(self, nz, G, D, r1_gamma=0.0):
        self.nz = nz
        self.G = G
        self.D = D
    ...
    def train(self, epochs, loader, criterion, optim_G, optim_D, loss='NS'):
        step = 0
...
def generate_images(model_path, dir_images_output, num_images=10000, batch_size=1000, truncated=None, device='cuda'):
    # load model
    netG = Generator(NZ, NGF, 3, True, SPECTRAL_NORM, NORMALIZATION, RANDOM_NOISE, USE_STYLE, PIXEL_NORM).to(device)
...
if __name__ == '__main__':
    ...
    # train
    clean_dir(DIR_IMAGES_OUTPUT)
    trainer = Trainer(NZ, netG, netD)
    trainer.train(EPOCHS, train_loader, criterion, optimizerG, optimizerD, loss=LOSS)
