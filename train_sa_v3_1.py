...
class Config():
    ...
    RANDOM_SEED = 5028
    PATH_PRETRAINED_G = '/data/cuong/result/motobike/sa_v3_1/61_G.pth'
    PATH_PRETRAINED_D = '/data/cuong/result/motobike/sa_v3_1/61_D.pth'
...
def generate_images(model_path, dir_images_output, num_images=10000, batch_size=1000, truncated=None, device='cuda'):
    ...
    for batch in range(int(num_images/batch_size)):
        ...
        if truncated is not None:
            ...
            z = torch.from_numpy(z[:batch_size*Config.NZ]).view(batch_size, Config.NZ)
            z = z.float().to(device)
        else:
...
if __name__ == '__main__':
    ...
    generate_seed(Config.RANDOM_SEED)
    print_params()
