...
DIR_IMAGES_INPUT = './data/motobike_1/'
...
class Generator(nn.Module):
    def __init__(self, nz, n_feat):
        ...
        self.res1    = ResBlock_G(16*n_feat, 16*n_feat, upsample=False)
        self.res1_up = ResBlock_G(16*n_feat, 16*n_feat, upsample=True)

        self.res2    = ResBlock_G(16*n_feat, 16*n_feat, upsample=False)
        self.res2_up = ResBlock_G(16*n_feat, 8*n_feat, upsample=True)

        self.res3    = ResBlock_G(8*n_feat, 8*n_feat, upsample=False)
        self.res3_up = ResBlock_G(8*n_feat, 4*n_feat, upsample=True)

        self.res4    = ResBlock_G(4*n_feat, 4*n_feat, upsample=False)
        self.res4_up = ResBlock_G(4*n_feat, 2*n_feat, upsample=True)

        self.res5    = ResBlock_G(2*n_feat, 2*n_feat, upsample=False)
        self.res5_up = ResBlock_G(2*n_feat, 1*n_feat, upsample=True)
    ...
    def forward(self, z):
        ...
        x = self.fc(z).view(batch_size,-1,4,4) #[*, 16*ch, 4, 4]

        x = self.res1(x) #[*, 16*ch, 4, 4]
        x = self.res1_up(x)#[*, 16*ch, 8, 8]

        x = self.res2(x) #[*, 16*ch, 8, 8]
        x = self.res2_up(x)#[*, 8*ch, 16, 16]

        x = self.res3(x)#[*, 8*ch, 16, 16]
        x = self.res3_up(x)#[*, 4*ch, 32, 32]

        x = self.res4(x)#[*, 4*ch, 32, 32]
        x = self.res4_up(x)#[*, 2*ch, 64, 64]

        x = self.res5(x)#[*, 2*ch, 64, 64]
        x = self.res5_up(x)#[*, 1*ch, 128, 128]

        x = self.conv(x)#[*, 3, 128, 128]
        x = torch.tanh(x)#[*, 3, 128, 128]
...
class Discriminator(nn.Module):
    def __init__(self, n_feat):
        super().__init__()
        self.conv3x3_1 = nn.utils.spectral_norm(conv3x3(3, n_feat)).apply(init_weight)

        self.res1_down = ResBlock_D(1*n_feat, 2*n_feat, downsample=True)
        self.res1      = ResBlock_D(2*n_feat, 2*n_feat, downsample=False)

        self.res2_down = ResBlock_D(2*n_feat, 4*n_feat, downsample=True)
        self.res2      = ResBlock_D(4*n_feat, 4*n_feat, downsample=False)

        self.res3_down = ResBlock_D(4*n_feat, 8*n_feat, downsample=True)
        self.res3      = ResBlock_D(8*n_feat, 8*n_feat, downsample=False)

        self.res4_down = ResBlock_D(8*n_feat, 16*n_feat, downsample=True)
        self.res4      = ResBlock_D(16*n_feat, 16*n_feat, downsample=False)

        self.res5_down = ResBlock_D(16*n_feat, 16*n_feat, downsample=True)
        self.res5      = ResBlock_D(16*n_feat, 16*n_feat, downsample=False)

        self.fc        = nn.utils.spectral_norm(nn.Linear(16*n_feat,1)).apply(init_weight)

    def forward(self, inputs):
        batch = inputs.size(0)      #(*,3,128,128)      #3

        h = self.conv3x3_1(inputs)  #[*, ch, 128, 128]

        h = self.res1_down(h)       #[*, ch*2, 64, 64]
        h = self.res1(h)            #[*, ch*2, 64, 64]

        h = self.res2_down(h)       #[*, ch*4, 32, 32]
        h = self.res2(h)            #[*, ch*4, 32, 32]

        h = self.res3_down(h)       #[*, ch*8, 16, 16]
        h = self.res3(h)            #[*, ch*8, 16, 16]

        h = self.res4_down(h)       #[*, ch*16, 8, 8]
        h = self.res4(h)            #[*, ch*16, 8, 8]

        h = self.res5_down(h)       #[*, ch*16, 4, 4]
        h = self.res5(h)            #[*, ch*16, 4, 4]

        h = torch.sum((F.relu(h)).view(batch,-1,4*4), dim=2) #[*, ch*16] #GlobalSumPool
        outputs = self.fc(h) #->(*,1)
...
#BigGAN
def run(lr_G=3e-4,lr_D=6e-4, beta1=0.0, beta2=0.999, nz=128, epochs=2,
        n_ite_D=1, ema_decay_rate=0.999, show_epoch_list=None, output_freq=10):

    netG = Generator(nz=nz, n_feat=24).to(device) #z.shape=(*,120)
    netD = Discriminator(n_feat=24).to(device)
