...
class Generator(nn.Module):
    def __init__(self, n_feat, codes_dim=20):
        ...
        self.fc   = nn.Sequential(
            nn.utils.spectral_norm(nn.Linear(codes_dim, 32*n_feat*4*4)).apply(init_weight)
        )
        self.res1 = ResBlock_G(32*n_feat, 32*n_feat, codes_dim, upsample=True)
        self.res2 = ResBlock_G(32*n_feat, 16*n_feat, codes_dim, upsample=True)
        self.res3 = ResBlock_G(16*n_feat,  8*n_feat, codes_dim, upsample=True)
        #self.attn = Attention(4*n_feat)
        self.res4 = ResBlock_G( 8*n_feat,  4*n_feat, codes_dim, upsample=True)
        self.res5 = ResBlock_G( 4*n_feat,  2*n_feat, codes_dim, upsample=True)
        self.conv = nn.Sequential(
            #nn.BatchNorm2d(2*n_feat).apply(init_weight),
            nn.LeakyReLU(),
            nn.utils.spectral_norm(conv3x3(2*n_feat, NC)).apply(init_weight),
        )
    ...
    def forward(self, z):
        ...
        x = self.fc(codes[0]) #->(*,32ch*4*4)
        x = x.view(batch,-1,4,4) #->(*,32ch,4,4)

        condition = codes[1]
        x = self.res1(x, condition)#->(*,32ch,8,8)

        condition = codes[2]
        x = self.res2(x, condition) #->(*,16ch,16,16)
        #x = self.attn2(x) #not change shape

        condition = codes[3]
        x = self.res3(x, condition) #->(*,8ch,32,32)
        #x = self.attn(x) #not change shape

        condition = codes[4]
        x = self.res4(x, condition) #->(*,4ch,64,64)
...
def validate_images_gen(netG, fixed_noise, dir_output):
    ...
    for i in range(gen_images.size(0)):
        image = np.array(gen_images[i, :, :, :]) * 255.0
        image = Image.fromarray(image.astype('uint8'), 'RGB')
        image = image.resize((128, 128), Image.ANTIALIAS)
        image.save(os.path.join(dir_output, '{}.png'.format(i)))

        #save_image(image, os.path.join(dir_output, '{}.png'.format(i)))
