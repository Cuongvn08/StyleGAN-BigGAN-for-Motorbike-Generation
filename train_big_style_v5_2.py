...
class StyleLayer(nn.Module):
    def __init__(self,nz=NZ):
        ...
        self.linear = nn.utils.spectral_norm(nn.Linear(nz,nz)).apply(init_weight)
...
class Generator(nn.Module):
    def __init__(self, n_feat, style_mlp=8):
        super().__init__()

        style_layers = []
        for i in range(style_mlp):
            style_layers.append(StyleLayer(NZ))
        self.style = nn.Sequential(*style_layers)

        self.fc   = nn.Sequential(
            nn.utils.spectral_norm(nn.Linear(NZ, 16*n_feat*4*4)).apply(init_weight)
        )
        self.res1 = ResBlock_G(16*n_feat, 16*n_feat, NZ, upsample=True)
        self.res2 = ResBlock_G(16*n_feat,  8*n_feat, NZ, upsample=True)
        self.res3 = ResBlock_G( 8*n_feat,   4*n_feat, NZ, upsample=True)
        #self.attn = Attention(4*n_feat)
        self.res4 = ResBlock_G( 4*n_feat,   2*n_feat, NZ, upsample=True)
        self.res5 = ResBlock_G( 2*n_feat,  1*n_feat, NZ, upsample=True)
    ...
    def forward(self, z):
        ...
        x = self.fc(z).view(batch,-1,4,4)   #->(*,16ch,4,4)
        x = self.res1(x, style)     #->(*,16ch,8,8)
        x = self.res2(x, style)     #->(*,8ch,16,16)
        x = self.res3(x, style)     #->(*,4ch,32,32)
        x = self.res4(x, style)     #->(*,2ch,64,64)
        x = self.res5(x, style)     #->(*,1ch,128,128)
...
def run(lr_G=3e-4,lr_D=6e-4, beta1=0.0, beta2=0.999, nz=120, epochs=2,
    ...
    netG = Generator(n_feat=NGF, style_mlp=STYLE_MLP).to(device) #z.shape=(*,120)
...
def print_params():
    ...
    printBoth(LOG, 'STYLE_MLP = {}'.format(STYLE_MLP))
...
def generate_images(model_path, dir_images_output, num_images=10000, batch_size=1000, truncated=None):
    #device = 'cpu'
    device = 'cuda'

    # load model
    netG = Generator(n_feat=NGF, style_mlp=STYLE_MLP).to(device)
