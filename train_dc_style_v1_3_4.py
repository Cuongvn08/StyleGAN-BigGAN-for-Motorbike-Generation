...
class Generator(nn.Module):
    def __init__(self,
        ...
        self.conv5 = nn.Sequential(
            nn.BatchNorm2d(1*nfeats),
            nn.LeakyReLU(),
            SN(nn.Conv2d(nfeats*1, nchannels, 1, 1, 0, bias=bias))
        )
...
class Discriminator(nn.Module):
    def __init__(self, nchannels, nfeats):
        ...
        self.linear = SN(nn.Linear(nfeats*16, 1, bias=False))
    ...
    def forward(self, x):
        ...
        x = torch.sum((F.leaky_relu(x,0.2)).view(x.shape[0],-1,8*8), dim=2)
